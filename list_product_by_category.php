...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM Products p, Category c WHERE p.status = 1 AND c.status = 1 AND is_accept = 1
              AND c.CategoryId = p.CategoriId AND CategoryId = '$id' LIMIT $startItem, $itemsPerPage";
    $Products = mysqli_query($conn, $query);

    // Truy vấn số lượng sản phẩm tổng cộng
    $totalQuery = "SELECT COUNT(*) as total FROM Products p, Category c WHERE p.status = 1 AND c.status = 1 AND is_accept = 1
    ...
    // Truy vấn dữ liệu của danh mục cụ thể
...
<section class="shop spad">
    ...
    <div class="row">
        ...
        <div class="shop__pagination">
             ...
             <?php } ?>
                <a href="?id=<?php echo $id; ?>&page=<?php echo min($totalPages, $currentPage + 1); ?>"><span class="arrow_carrot-right"></span></a>
        </div>
        </div>
        <div class="col-lg-6 col-md-6 col-sm-6">
